...
//	location.href = "search?searchtitle="+searchtitle+"&searchvalue="
//				+searchvalue+"&boardname="+a
//	}
...
<jsp:include page="../css/header2.jsp"/>
	...
	<div>
		...
		<c:when test="${searchvalue == null }">
			<c:set var="searchvalue" value="자유게시판" />
		</c:when>
		<c:otherwise>
	...
	<a href="totalboard">totalboard이동</a>
		<table border="1">
			...
			<c:forEach var="bdto" items="${freelist }">
		...
		<c:choose>
			<c:when test="${start > 1}">
				<button type="button"
					onclick="location.href='free?stnum=${start-1 }'">이전
				</button>
			</c:when>
			<c:otherwise>
		...
		<c:forEach begin="1" end="${totalpage}" step="1" var="cnt">
			<a href="free?stnum=${cnt}">[${cnt}]</a>
		</c:forEach>
		<c:choose>
			<c:when test="${start < totalpage}">
				<button type="button"
					onclick="location.href='free?stnum=${start+1 }'">다음
				</button>
			</c:when>
			<c:otherwise>
		...
		<input type="button" value="글쓰기"
			onclick="location.href='writeboard?boardname=자유게시판'"> 
